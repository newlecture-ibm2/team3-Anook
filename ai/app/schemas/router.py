...
class RouterOutputSchema(BaseModel):
    ...
    """
    mode: str = Field(
        description="처리 모드 (TASK: 직원이 출동해야 하는 업무 요청, CHITCHAT: 단순 인사/일상 대화, CLARIFICATION: 의미가 모호하여 되물어야 하는 경우, INFO: 단순 정보 문의, CANCEL: 요청 취소)"
    )
    domain: Optional[str] = Field(
        default=None, 
        description="담당 부서 코드 (HK, FB, FACILITY, CONCIERGE, FRONT, COMMON). mode가 TASK 또는 INFO일 때 값이 존재함."
    )
