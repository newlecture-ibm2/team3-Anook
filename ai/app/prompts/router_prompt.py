...
8. **CANCEL** (Request Cancellation):
   ...
   - **PARTIAL CANCELLATION OF MULTI-ITEM ORDERS (CRITICAL)**: Some requests in the active list may contain MULTIPLE items bundled together (e.g., summary: "아이스 아메리카노 3개, 스테이크 샌드위치 1개 주문"). If the user wants to cancel ONLY SOME items from such a bundled request (e.g., "아메리카노만 취소해줘"), you MUST NOT route to "CANCEL" — cancelling would remove the ENTIRE order including items the user wants to keep. Instead, route to "DEPARTMENT" with `domain: "FB"` so the FB agent can handle it as an ORDER_MODIFY (removing the cancelled item while preserving the rest).
...
- **OFFER REJECTION RULE**: 
  ...
  - **Case 2 (Task Confirmation/Details - IN-PROGRESS)**: If the AI is still asking for missing details (e.g., "어느 꽃집에서 배달해 드릴까요?") or asking for final confirmation BEFORE registration (e.g., "Shall I process the delivery?"), and the user says "No", "안할게요", or "취소", you MUST classify it as "NON_ACTIONABLE". DO NOT classify it as "CANCEL", because "CANCEL" will send a destructive API request to delete an already registered task in the database.
...
- **ESCALATION CONFIRMATION RULE**: If the last AI message in `[과거 대화 맥락]` asked if the guest wants to connect to the front desk or needs intervention (e.g., "프런트로 연결해 드릴까요?", "프런트 데스크의 직접적인 조치나 확인이 필요하신 상황일까요?", "connect you to the front desk"), and the guest agrees (e.g., "네", "조치해줘", "yes", "응"), you MUST classify it as "FRONT_ESCALATION" with domain "FRONT" and explicitly set `create_ticket: true`. IMPORTANT: If the escalation is specifically because the user wants more information (e.g., the AI asked "자세한 정보가 필요하시면 프런트로 연결해 드릴까요?"), you MUST include the exact text "INFO_ESCALATION" in the `reasoning` field.
