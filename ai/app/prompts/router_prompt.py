...
- **CONTEXT RESET RULE (STRICT)**: If the last AI message in `[과거 대화 맥락]` indicates that a previous request was already COMPLETED, ANSWERED, CONFIRMED, or ESCALATED (e.g., "접수되었습니다", "완료했습니다", "연결해 드릴게요"), you MUST treat the user's current message as a completely NEW and INDEPENDENT request. 
...
- **OFFER REJECTION RULE**: 
  ...
  - **Case 2 (Task Confirmation - IN-PROGRESS)**: If the AI just asked for confirmation (e.g., "접수해 드릴까요?") and the user says "No", classify it as "CANCEL" with the relevant domain.
