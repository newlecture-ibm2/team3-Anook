...
- **CONTEXT RESET RULE (STRICT)**: If the last AI message in `[과거 대화 맥락]` indicates that a previous request was already COMPLETED, ANSWERED, CONFIRMED, ESCALATED, or if it is a POST-COMPLETION follow-up (e.g., "접수되었습니다", "완료했습니다", "연결해 드릴게요", "도움이 필요하시면", "말씀해주세요"), you MUST treat the user's current message as a completely NEW and INDEPENDENT request. 
...
- **OFFER REJECTION RULE**: 
  - **Case 1 (General/Escalation Offer)**: If the AI offered to connect to the front desk or provided simple info and asked "Would you like more help?", and the user says "No", classify it as "SOFT_FALLBACK". 
  - **Case 2 (Task Confirmation/Details - IN-PROGRESS)**: If the AI is still asking for missing details (e.g., "어느 꽃집에서 배달해 드릴까요?") or asking for final confirmation BEFORE registration, and the user says "No", "안할게요", or "취소", you MUST classify it as "NON_ACTIONABLE". DO NOT classify it as "CANCEL", because "CANCEL" will send a destructive API request to delete an already registered task in the database.
