...
STATIC_MESSAGES = {
    "ESCALATION": {
        ...
        "en": "I apologize, but I'm having trouble understanding. I will connect you to a front desk agent immediately.",
        "ja": "申し訳ありません。正確な把握が難しいため、すぐにフロントデスクのスタッフにおつなぎいたします。",
        "zh": "抱歉，我不太明白您的意思。我将立即为您转接前台人工客服。"
    },
    "CLARIFICATION": {
        "ko": "죄송합니다, 조금 더 자세히 말씀해 주시겠어요? 어떤 도움이 필요하신지 알려주시면 바로 도와드리겠습니다.",
        "en": "I'm sorry, could you please provide more details? Let us know what you need so we can assist you.",
        "ja": "申し訳ありませんが、もう少し詳しく教えていただけますか？どのようなご用件かお知らせいただければ、すぐに対応いたします。",
        "zh": "抱歉，您能再详细说明一下吗？请告诉我们您需要什么帮助，我们会立即为您处理。"
    },
    "MISSING_AGENT": {
        "ko": "네, 알겠습니다. 담당 부서에 요청을 전달하겠습니다. 잠시만 기다려 주세요.",
        "en": "Understood. We will forward your request to the relevant department. Please wait a moment.",
        "ja": "はい、承知いたしました。担当部署にお繋ぎします。少々お待ちください。",
        "zh": "好的，我们会将您的请求转交给相关部门。请稍候。"
    },
    "CANCEL": {
        "ko": "네, 가장 최근 요청을 취소 처리하겠습니다.",
        "en": "Understood. We will cancel your most recent request.",
        "ja": "はい、最新のリクエストをキャンセルいたします。",
        "zh": "好的，我们将取消您最近的请求。"
    },
    "NOT_FOUND": {
        "ko": "죄송합니다, 해당 정보를 찾지 못했습니다. 프론트 데스크(내선 0번)로 문의해 주세요.",
        "en": "I apologize, but I couldn't find that information. Please contact the front desk (Dial 0).",
        "ja": "申し訳ありませんが、該当する情報が見つかりませんでした。フロントデスク（内線0番）までお問い合わせください。",
        "zh": "抱歉，我找不到相关信息。请联系前台（拨打 0）。"
    },
    "ERROR": {
        "ko": "요청을 처리하는 중 문제가 발생했습니다.",
        "en": "An error occurred while processing your request.",
        "ja": "リクエストの処理中に問題が発生しました。",
        "zh": "处理您的请求时发生错误。"
    }
}

def get_msg(key: str, lang: str) -> str:
    l = lang.lower()[:2] if lang else "ko"
    msgs = STATIC_MESSAGES.get(key, {})
    return msgs.get(l, msgs.get("ko", ""))
...
async def analyze_message(request: AnalyzeRequest) -> Dict[str, Any]:
    ...
    if is_clarification:
        ...
        if consecutive_questions >= 3:
            ...
            response = {
                "guest_reply": get_msg("ESCALATION", request.language),
...
async def _analyze_message_core(request: AnalyzeRequest) -> Dict[str, Any]:
    print(f"\n[Analyze] 📩 요청 수신 - Room: {request.room_no}, Lang: {request.language}, Text: '{request.text}'")
    ...
    try:
        ...
        return _fallback_response(get_msg("ERROR", request.language))
    ...
    if primary.mode == "TASK" and primary.domain:
            ...
            try:
                ...
                if agent_confidence < 0.4:
                    ...
                    if "직원" not in final_guest_reply:
                        final_guest_reply = get_msg("ESCALATION", request.language)
        ...
        response = {
            "guest_reply": get_msg("MISSING_AGENT", request.language),
        ...
        try:
            raw = call_gemini(
                ...
                system_instruction=(
                    ...
                    f'고객의 인사나 일상 대화에 따뜻하고 간결하게 고객의 언어({request.language})로 답변하세요. '
                    f'{{"reply": "답변내용"}} 형식의 JSON으로만 출력하세요.'
                ),
            )
    ...
    if primary.mode == "CLARIFICATION":
        response = {
            "guest_reply": get_msg("CLARIFICATION", request.language),
            "summary": "추가 확인 필요",
            "domain_code": None,
            "priority": "NORMAL",
            "entities": {},
        ...
        try:
            ...
            if rag_results:
                ...
                info_prompt = (
                    ...
                    f"아래 제공된 [호텔 지식]만을 바탕으로 고객의 언어({request.language})로 친절하게 답변하세요. "
                    f"만약 [호텔 지식]에 고객의 질문에 대한 명확한 답이 없다면, 절대 유추하거나 지어내지 말고 "
                    f"'{get_msg('NOT_FOUND', request.language)}'라고 답변하세요.\n\n"
                    f"[호텔 지식]\n{knowledge}"
                )
                raw = call_gemini(
                    prompt=info_prompt, 
                ...
                guest_reply = get_msg("NOT_FOUND", request.language)
        except Exception as e:
    ...
    if primary.mode == "CANCEL":
        response = {
            "guest_reply": get_msg("CANCEL", request.language),
            "summary": "요청 취소",
            "domain_code": None,
            "priority": "NORMAL",
            "entities": {},
