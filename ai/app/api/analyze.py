...
async def _analyze_message_core(request: AnalyzeRequest) -> List[Dict[str, Any]]:
    ...
    for primary in router_results:
        ...
        # STEP 3-h: BILLING_INQUIRY → 통합 이용 금액 조회 (실시간 Redis + DB 영수증)
        if primary.route_type == "BILLING_INQUIRY":
            guest_reply = _handle_unified_billing_inquiry(request.room_no, request.text, request.room_inventory)
            response = {
                "guest_reply": guest_reply,
                "summary": "통합 이용 금액 조회",
                "domain_code": None,
                "priority": "NORMAL",
                "entities": {"action": "BILLING_INQUIRY"},
                "confidence": primary.confidence,
                "action": "BILLING_INQUIRY",
...
def _handle_unified_billing_inquiry(room_no: str, user_message: str, room_inventory: dict = None) -> str:
